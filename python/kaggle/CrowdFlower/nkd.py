...
def index_corpus():
    ...
    d = defaultdict(int)
    for e, row in enumerate(csv.DictReader(open("train.csv", 'r', newline='', encoding='utf8'))):
        s = clean(
            "medianrellabel%s %s %s" % (row["median_relevance"], row["product_description"], row["product_title"]))
        d = index_document(s, d)
    for e, row in enumerate(csv.DictReader(open("test.csv", 'r', newline='', encoding='utf8'))):
...
def multiple_choice(d, question, anchor, choices):
    ...
    q = """<li class="pane"><h3>%s</h3><ul>%s</ul></li>""" % (question,
                                                              "".join(
                                                                  ["<li><span>%s</span>%s</li>" % (round(w[0], 3), w[1])
                                                                   for w in
                                                                   sorted([(nkd(f, anchor, d), f) for f in choices])]))
    return q
